            ...
            if label in participants_to_highlight and label not in special_labels:
                annotations.append(
                    ax.text(
                        ...
                        fontsize=11,
                        fontname='Arial',
                        bbox=dict(boxstyle="round,pad=0.15", edgecolor="black",
                                  facecolor="white", alpha=0.5), zorder=2))
        adjust_text(annotations,ax=ax, arrowprops=dict(arrowstyle="->", color='black', alpha=0.5))
        for special_label, offset in special_labels.items():
            if special_label in labels.values:
                ...
                for idx in special_indices:
                    ax.annotate(
                        special_label,
                        (x_data.iloc[idx], y_data.iloc[idx]),
...
def plot_quadratic_regression(
    x, y, labels, hue=None,
    hue_order=None, palette=None,
    xlabel='', ylabel='', title='',
    filename=None, ukraine=False,
    legend_title='Sociality',
    annotate=True, figsize=(14, 6)):
    ...
    plt.plot(x_sorted, y_pred, color='black')  # black line is fine
    plt.xlabel(xlabel, fontsize=11, fontname='Arial')
    plt.ylabel(ylabel, fontsize=11, fontname='Arial')
    if title:
        plt.title(title, fontsize=11, fontname='Arial')
    plt.grid(True)
    if hue is not None:
        plt.legend(title=legend_title, bbox_to_anchor=(1.21, 1), prop={'family': 'Arial', 'size': 11})
    ...
    print(f"{title}")
    print(
        f"The overall model was{' not' if p_val > 0.05 else ''} significant, "
        f"F({k-1}, {n-k-1}) = {f_stat:.4f}, p = {p_val:.4f}, "
        f"with an R² value of {r_squared:.4f}, indicating that {r_squared * 100:.2f}% "
        f"of the variance in {ylabel.lower()} was explained by the model."
    )
...
participants_to_highlight = [
    'P1', 'P2', 'P3', 'P7', 'P8', 'P9', 'P10', 'P11', 'P12', 'P13',
    'P14', 'P15', 'P17', 'P18', 'P19', 'P20', 'P22', 'P30']
...
participants_to_highlight = [
    'P1', 'P5', 'P6', 'P9', 'P10', 'P11', 'P12', 'P15', 'P16', 'P17',
    'P18', 'P20', 'P22', 'P23', 'P31', 'P32', 'P51', 'P52', 'P12']
...
palette_full = {
    'low sociality': 'black',
    'medium sociality': 'dimgray',
    'developed sociality': 'darkgray',
...
ax = sns.scatterplot(
    ...
    style=sociality1,
    s=100,
    hue_order=legend_order_1,
    style_order=legend_order_1,
    zorder=1)

# quadratic regression
z1 = np.polyfit(x1, y1, 2)
p1 = np.poly1d(z1)
x_smooth = np.linspace(np.min(x1), np.max(x1), 300)  # smoothing
...
plt.xlabel('Conspiracy mentality questionnaire', fontsize=11, fontname='Arial')
plt.gcf().text(0.5, 0.03, '(CMQ)', ha='center', va='top', fontsize=11, fontname='Arial')
plt.ylabel('Complexity', fontsize=11, fontname='Arial')
plt.legend(title='Sociality', prop={'family': 'Arial', 'size': 11})
plt.grid(True)
...
plot_quadratic_regression(
    ...
    hue=data_data['sociality_label'],
    hue_order=legend_order,
    palette=palette_full,
    xlabel='Conspiracy mentality questionnaire (CMQ)',
    ylabel='Complexity',
    title='',
...
plot_quadratic_regression(
    ...
    hue_order=legend_order,
    palette=palette_full,
    xlabel='General conspiracy beliefs (GCB)',
    ylabel='Complexity',
    title='',
...
plot_quadratic_regression(
    ...
    hue_order=legend_order,
    palette=palette_full,
    xlabel='Ukraine conspiracy beliefs (UCB)',
    ylabel='Complexity',
    title='',
    filename='study2_UCB.jpg',
    ukraine=True)
